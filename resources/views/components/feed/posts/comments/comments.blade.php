@foreach ($comments as $comment)
    ...
    <div class="post px-4 pt-3 pb-2 d-flex" onclick="openThread('{{ route('show.post', [$comment->id]) }}')">
        ...
        <section class="ps-3 d-flex flex-column w-100">
            <div class="d-flex w-100 align-items-center">
                ...
                <div class="time"><strong class="mx-2">·</strong>{{$comment->time_ago }}</div>
            </div>
            ...
            <section class="d-flex align-items-center justify-content-between mt-3 mb-1 w-50" id="actions">
                <span class="btn-action comment" onclick="openThread('{{ route('show.post', [$comment->id]) }}')">
                    ...
                    @if ($comment->comments->count() > 0)
                        <span class="commentsCount text-muted">{{$comment->comments->count()}}</span>
                    @endif
